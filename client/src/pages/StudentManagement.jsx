...
const StudentManagement = () => {
  ...
  const handleBulkCreate = async () => {
    try {
      ...
      // Add sessionId if available
      const bulkDataWithSession = { ...bulkData };
      if (sessionId) {
      ...
      const response = await fetch(`${import.meta.env.VITE_API_URL || (import.meta.env.VITE_API_URL || 'http://localhost:8000/api')}/data/students/bulk-create`, {
        ...
        body: JSON.stringify(bulkDataWithSession)
      });
  ...
  const handleFileUpload = async () => {
    ...
    try {
      ...
      formDataObj.append('csvFile', uploadFile);
      formDataObj.append('sendCredentials', bulkData.sendCredentials);
