...
router.post('/timetable/generate', [
  ...
  try {
      ...
      if (course.assignedTeachers && course.assignedTeachers.length > 0) {
        course.assignedTeachers.forEach(at => {
          ...
          if (teacherId) {
            ...
            teacherSubjectMap[teacherId].push({
              courseId: course._id.toString(),
              courseCode: course.code,
              courseName: course.name,
              sessionTypes: at.sessionTypes || ['Theory', 'Practical', 'Tutorial']
            });
          }
        });
      }
    ...
    logger.info('[SESSION GENERATE] Teacher-subject mapping built', {
      teachersWithSubjects: Object.keys(teacherSubjectMap).length
    });

    // Step 4: Prepare settings from session config
    const settings = {
      algorithm: selectedAlgorithm,
      ...
      enforceBreaks: true,
      balanceWorkload: session.config?.compactSchedulePreference || true,
      ...session.config,
      teacherSubjectMap
    };

    // Step 5: Run optimization
    logger.info('[SESSION GENERATE] Starting optimization', { algorithm: selectedAlgorithm });
    ...
    logger.info('[SESSION GENERATE] Optimization completed', {
      success: result.success,
      hasSolution: !!result.solution
    });

    if (!result.success || !result.solution || result.solution.length === 0) {
      // Update session status to failed
      await TimetableSession.findByIdAndUpdate(sessionId, {
        status: 'ready',
        config: { ...session.config, lastError: result.reason }
      });

      return res.status(400).json({
        success: false,
        message: 'Failed to generate timetable',
        reason: result.reason || 'No feasible solution found'
      });
    }

    // Step 6: Validate hard constraints
    logger.info('[SESSION GENERATE] Validating constraints');
    const validationResult = validateHardConstraints(result.solution, teachers, classrooms, courses);

    // Step 7: Calculate soft scoring
    logger.info('[SESSION GENERATE] Calculating soft scores');
    const softScores = calculateSoftScores(result.solution, teachers, classrooms, courses, session.config || {});
    ...
    const enrichedSchedule = enrichSchedule(result.solution, teachers, classrooms, courses);
    ...
    const timetable = new Timetable({
      ...
      generationSettings: {
        ...
        enforceBreaks: settings.enforceBreaks,
        balanceWorkload: settings.balanceWorkload
      },
      ...
      statistics: calculateStatistics(result.solution, teachers, classrooms),
      createdBy: req.user?.userId
    });
...
function calculateSoftScores(schedule, teachers, classrooms, courses, config) {
  ...
  // Calculate student convenience (gap minimization)
  // Group by division/program and calculate gaps
  const studentConvenience = 85; // Placeholder - would need more detailed analysis

  // Teacher satisfaction (based on workload and preferences)
